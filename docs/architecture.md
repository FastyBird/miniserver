...
`public/index.php` is the single entry point for both the API and the UI. It inspects `$_SERVER['REQUEST_URI']` against `FastyBird\Core\Constants\Metadata\Constants::ROUTER_API_PREFIX`: a match is routed to the ReactPHP-based `FastyBird\Core\Server\HttpServer\Application`; everything else goes to the Nette application.
