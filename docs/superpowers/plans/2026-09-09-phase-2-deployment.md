...
Expected: `Successfully tagged fastybird-dev-php-check:latest` (or the buildkit equivalent). This image installs `bcmath calendar ctype curl dom exif fileinfo gmp intl mbstring mysqli opcache pcntl pdo pdo_mysql pdo_pgsql pgsql phar simplexml sockets xml xmlwriter xsl zip` plus `apcu` and `xdebug` through pecl. Note that the file being moved does **not** currently install the `gd` extension and does **not** install `libfreetype6-dev`/`libjpeg62-turbo-dev`; this is a pre-existing gap (composer.json hard-requires `ext-gd`) that this task carries forward unchanged and does not fix, because Task 4 is scoped to relocation only — closing that gap is out of scope for this phase.
