...
- `repositories`: `{"type": "path", "url": "src/FastyBird/*/*"}` plus the existing `mathsolver/mathsolver` VCS entry. **Do NOT set `"options": {"symlink": true}`.** It was in an earlier draft of this spec and is wrong: an explicit `symlink: true` unconditionally overrides `COMPOSER_MIRROR_PATH_REPOS`, so the production image would symlink path packages into `vendor/` instead of copying them, and the symlinks break the moment the image is built. Verified against Composer's own source during Phase 3. Omitting the option leaves symlinking as the local default while letting the environment variable force copying where it matters.
...
2. Delete `FastyBird/miniserver-old` and the 25 split repositories: `application`, `exchange`, `tools`, `metadata-library`, `web-ui-library`, `devices-module`, `accounts-module`, `triggers-module`, `ui-module`, `fb-mqtt-connector`, `homekit-connector`, `modbus-connector`, `ns-panel-connector`, `shelly-connector`, `sonoff-connector`, `tuya-connector`, `viera-connector`, `virtual-connector`, `zigbee2mqtt-connector`, `couchdb-plugin`, `redisdb-plugin`, `rabbitmq-plugin`, `web-server-plugin`, `ws-server-plugin`, `virtual-thermostat-addon`. **`FastyBird/libraries-patches` must NOT be deleted**, in this phase or after it. Established empirically during Phase 1 on 2026-09-10: three external dependencies declare their own `extra.patches` entries pointing at raw URLs in that repository. `fastybird/json-api` and `fastybird/datetime-factory` each patch `nette/utils`; `fastybird/simple-auth` patches both `nette/utils` and `nettrine/orm`. Those declarations are honoured at install time regardless of the root's `extra.enable-patching: false`, and a dependency's URL wins over a root entry with the same description key. A cold install on 2026-09-10 confirmed the exact split: nine of eleven applied patches resolve from `tools/patches/`, while `nette/utils` and `nettrine/orm` are still fetched over the network. The `nettrine/orm` case is the sharper one, because the root DOES declare it locally and `fastybird/simple-auth` overrides it, leaving the vendored file unused. Vendoring therefore did not make the project independent of that repository. Deleting it would break `composer install` on every future checkout. It becomes eligible for deletion only after Phase 6 updates or absorbs those three libraries and a clean `composer install` shows no remaining reference to it. `FastyBird/.github` stays for the organization profile and README assets.
...
- **The Nette application has no registered routes, so every non-API web request 404s.** `src/FastyBird/Core/Application/src/Router/AppRouter.php` defines `createRouter()` with a route for `/` mapping to `App:Default:default`, but nothing registers it: there is no `services: router:` entry and no `application: mapping:` in `Core/Application/config/*.neon` or the root runtime config, and `git log -S'AppRouter'` over those paths returns nothing, so it was never wired. Confirmed against a running production container on 2026-09-10: `/` returns 404 from Nette (`X-Powered-By: Nette Framework 3`), while `/favicon.ico` returns 200 from nginx and `/api/v1` returns a correct JSON:API 404, so both stacks are otherwise healthy. Pre-existing and out of scope for the merge, which must not change behaviour. Whoever wires the SPA shell must register the router and add a presenter mapping; until then, no deployment smoke test should assert a 200 on `/`.

- **`var/config` lives inside the `/app/var` volume, which breaks two upgrade paths.** The production image copies `var/config` in and then declares `VOLUME /app/var` over it. A fresh named volume is seeded from the image, which is the only case tested. Upgrading over an existing volume keeps the OLD config, so configuration shipped in a new release never reaches an upgraded install. A bind mount is worse: binds are never seeded, so `/app/var/config` does not exist, `FB_CONFIG_DIR` resolves to `false`, and no application config loads at all, silently. Phase 3 moves configuration to `config/` outside the data volume, which resolves this; until then, do not bind-mount `/app/var`.

- **Deployment has no operator documentation.** A fresh `docker compose up -d` produces a container that is up and healthy, with an empty database and no schema. That schema creation is a manual step exists only as an `echo` in the entrypoint that scrolls past in the container log. Phase 4 owns writing `docs/deployment.md`.

- **The Debian package does not install what it references.** `build/debian/make_deb.sh` copies only `public/` and `vendor/` into the package, never `bin/` or `src/`, yet the post-install script symlinks `bin/fb-console.php` and the systemd unit executes it. The package as ported cannot work. Its dependency list is also unvalidated: it names a `php8.2-json` package that does not exist (JSON has been in php-common since 8.0), lists `php8.2-cli` twice, and omits `php8.2-gmp`, `php8.2-zip` and sockets although the application declares all three. This is consistent with the packaging being marked unsupported, but the README should say the package is known non-functional rather than implying only a PHP version bump was needed.

- **Development and production images diverge in ways that will bite.** Development installs apcu, xdebug, mysqli, the PostgreSQL drivers, xsl, calendar, exif and xmlwriter; production installs none of them. All declared `ext-*` requirements are satisfied in both, so this only bites undeclared usage — but a `local.neon` selecting APCu cache storage would work in development and hard-fail in production. Memory limits differ (512M against 256M), opcache CLI JIT is off in development and on in production with a 256M buffer per process, production nginx drops development's 600-second FastCGI timeout, and production pins MariaDB 10.11 while development floats on latest and Doctrine is told the server is 5.7 in both.

- **No base image is pinned by digest.** `node:20`, `composer:2`, `php:8.2-fpm` and the MariaDB images all float. For a project whose premise is a frozen reproducible baseline, the images drift between builds.

- **The lockfile contains one floating tag.** `yarn.lock` pins `vue-component-type-helpers` under both `^3.3.9` and the literal `latest` tag, the only such entry in the file. Yarn resolves both to one version and emits a non-determinism warning on every install. Harmless today, but a lockfile carrying `latest` does not fully freeze the tree, which is exactly the class of drift that produced the Shelly and jsona defects. Clean it up when dependencies are next touched.
