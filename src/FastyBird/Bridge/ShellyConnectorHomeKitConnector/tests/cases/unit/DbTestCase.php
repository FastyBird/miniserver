...
abstract class DbTestCase extends TestCase
{
	...
	private function setupDatabase(): void
	{
		if (!$this->isDatabaseSetUp) {
			...
			/** @var list<ORM\Mapping\ClassMetadata<Entities\CrudEntity>> $metadatas */
	...
	private function loadFromFile(DBAL\Connection $db, string $file): void
	{
		...
		if ($handle === false) {
			throw new ShellyConnectorHomeKitConnectorExceptions\InvalidArgument(sprintf('Cannot open file "%s".', $file));
		}
	...
	private function executeFixtureStatement(DBAL\Connection $db, string $file, string $sql): void
	{
		try {
			...
			throw new ShellyConnectorHomeKitConnectorExceptions\InvalidArgument(
				sprintf(
