 ...
 */
final class JsonApiMiddleware implements Server\MiddlewareInterface
{
	...
	): Message\ResponseInterface
	{
		try {
			...
			if ($ex instanceof ApiExceptions\JsonApi) {
				$response = $response->withStatus($ex->getCode());

				if ($ex instanceof ApiExceptions\JsonApiError) {
					$content = $this->getEncoder()
						->encodeError($ex->getError());

					$response->getBody()
						->write($content);
				} elseif ($ex instanceof ApiExceptions\JsonApiMultipleError) {
					$content = $this->getEncoder()
