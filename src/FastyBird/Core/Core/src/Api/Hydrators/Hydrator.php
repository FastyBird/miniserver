...
abstract class Hydrator
{
	...
	private ApiExceptions\JsonApiMultipleError $errors;
	...
	{
		$this->errors = new ApiExceptions\JsonApiMultipleError();
	}
		...
		foreach ($entityFields as $fieldName) {
			...
			if (in_array(ORM\Mapping\OneToOne::class, $propertyAttributes, true)) {
				...
				if (is_string($className) && class_exists($className)) {
					$fields[] = new Fields\SingleEntityField(
						$className,
						false,
						$mappedKey,
						$isRelationship,
				...
				// Check if class is callable
				if (is_string($className) && class_exists($className)) {
					$fields[] = new Fields\CollectionField(
						$className,
						true,
						$mappedKey,
						$isRelationship,
					...
					if ($typesFound > 1) {
						...
						try {
							...
							if (
								...
								$fields[] = new Fields\DateTimeField(
									$isNullable,
									$mappedKey,
									$fieldName,
									$isRequired,
								...
								$fields[] = new Fields\BackedEnumField(
									$this->translator,
									$className,
									$isNullable,
									$mappedKey,
						...
						$fields[] = new Fields\BooleanField(
							$this->translator,
							$isNullable,
							$mappedKey,
							$fieldName,
	...
	): array
	{
		...
		foreach ($entityMapping as $field) {
			...
			if ($value === null && $field->isRequired() && $isNew) {
				...
				if ($field instanceof Fields\SingleEntityField) {
