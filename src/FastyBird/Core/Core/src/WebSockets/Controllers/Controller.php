 ...
 */
abstract class Controller implements RequestController
{
	...
	private Responses\ControllerResponse $response;
	 ...
	 * @throws WebSocketsExceptions\Abort
	 */
	public function sendResponse(Responses\ControllerResponse $response): void
	{
	...
	): void
	{
		...
		foreach ($rm->getParameters() as $param) {
			[$type, $isClass] = Reflection::getParameterType($param);
			$name = $param->getName();
	...
	protected function shutdown(Responses\ControllerResponse $response): void
	{
	...
	protected function tryCall(string $method, array $params): bool
	{
		...
		if ($rc->hasMethod($method)) {
			...
			if ($rm->isPublic() && !$rm->isAbstract() && !$rm->isStatic()) {
				...
				$rm->invokeArgs($this, Reflection::combineArgs($rm, $params));
