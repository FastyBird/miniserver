...
final class ControllerFactory implements IControllerFactory
{
	...
	public function getControllerClass(string &$name): string
	{
		...
		if (!$reflection->implementsInterface(RequestController::class)) {
			throw new Exceptions\InvalidController(
				sprintf(
					'Cannot load controller "%s", class "%s" is not FastyBird\\Core\\WebSockets\\Controllers\\RequestController implementor.',
					$name,
					$class,
				),
			);
