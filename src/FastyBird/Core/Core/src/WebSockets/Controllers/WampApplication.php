...
final class WampApplication extends Application implements IWampApplication
{
	...
	): void
	{
		...
		try {
			...
			if ($json === null || !is_array($json) || $json !== array_values($json)) {
				throw new CoreExceptions\InvalidArgument('Invalid WAMP message format');
			}
			...
			switch ($json[0]) {
				...
				case self::MSG_PUBLISH:
					...
					break;
				default:
					throw new CoreExceptions\InvalidArgument('Invalid WAMP message type');
			}
		} catch (WebSocketsExceptions\Terminate $ex) {
			throw $ex;
		} catch (Throwable $ex) {
	...
	): Handshake\IRequest
	{
		...
		$parsedAction = $url->getQueryParameter(Controller::ACTION_KEY);

		if ($parsedAction === null || $parsedAction === Controller::DEFAULT_ACTION) {
			$url->setQueryParameter(Controller::ACTION_KEY, $action);
		}
