...
namespace FastyBird\Library\WebSockets\Protocols\RFC6455;
...
final class Frame implements Protocols\IFrame
{
	 ...
	 */
	private int $secondByte = -1;

	public function __construct(string|null $payload = null, bool $final = true, int $opCode = self::OP_TEXT)
	{
		if ($payload === null) {
			return;
		}
	...
	public function isFinal(): bool
	{
		if ($this->firstByte === -1) {
			throw new UnderflowException(
				'Not enough bytes received to determine if this is the final frame in message',
			);
		}
	...
	public function isMasked(): bool
	{
		if ($this->secondByte === -1) {
			throw new UnderflowException(
				sprintf('Not enough bytes received (%s) to determine if mask is set', $this->bytesReceived),
			);
		}
	...
	public function maskPayload(string|null $maskingKey = null): void
	{
		...
		if (strlen($maskingKey) !== self::MASK_LENGTH) {
			throw new Exceptions\InvalidArgument(sprintf('Masking key must be %s characters', self::MASK_LENGTH));
		}
		...
		$this->data = substr_replace(
			$this->data,
			$this->applyMask($maskingKey),
			$this->getPayloadStartingByte(),
	 ...
	 */
	public function applyMask(string $maskingKey, string|null $payload = null): string
	{
		if ($payload === null) {
			if (!$this->isCoalesced()) {
