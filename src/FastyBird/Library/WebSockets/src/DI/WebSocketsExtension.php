...
namespace FastyBird\Library\WebSockets\DI;

use FastyBird\Library\WebSockets\Application;
use FastyBird\Library\WebSockets\Clients;
use FastyBird\Library\WebSockets\Commands;
use FastyBird\Library\WebSockets\Events;
use FastyBird\Library\WebSockets\Logger;
use FastyBird\Library\WebSockets\Router;
use FastyBird\Library\WebSockets\Server;
use Nette;
use Nette\DI;
use Nette\Schema;
use Psr\Log;
...
final class WebSocketsExtension extends DI\CompilerExtension
{
	...
	public function loadConfiguration(): void
	{
		...
		foreach ($configuration->routes as $mask => $action) {
			$router->addSetup('$service[] = new FastyBird\Library\WebSockets\Router\Route(?, ?);', [$mask, $action]);
		}
		...
		if ($configuration->loop === null) {
			$loop = $builder->getByType(React\EventLoop\LoopInterface::class) === null ? $builder->addDefinition(
				$this->prefix('server.loop'),
			)
				->setType(React\EventLoop\LoopInterface::class)
				->setFactory('React\EventLoop\Factory::create') : $builder->getDefinitionByType(
	...
	public function beforeCompile(): void
	{
		...
		if (
			...
			$applicationType = $builder->getByType(Application\Application::class);
